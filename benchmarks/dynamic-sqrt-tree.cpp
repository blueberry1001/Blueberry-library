// Reproducible microbenchmark: see docs/development/dynamic-sqrt-tree-comparison.md.
#include <algorithm>
#include <chrono>
#include <cstdint>
#include <cstdlib>
...
void measure(const std::string& name, const std::vector<long long>& values,
  ...
  mix(checksum, static_cast<std::uint64_t>(tree.all_prod()));
...
int main(int argc, char** argv) {
  if (argc != 6) {
    std::cerr << "usage: benchmark <static|dynamic|acl> N Q update_percent seed\n";
    return 2;
  }
  ...
  if (n <= 0 || q < 0 || rate < 0 || rate > 100 || (implementation == "static" && rate != 0)) return 2;
  ...
  for (int i = 0; i < q; ++i) {
    ...
    if (!update && l > r) std::swap(l, r);
    queries.push_back({l, r, value, update});
    mix(input_hash, l); mix(input_hash, r); mix(input_hash, value); mix(input_hash, update);
  }
  if (implementation == "static") measure<blueberry::SqrtTree<long long, sum_op, sum_identity>>(implementation, values, queries, rate, seed, input_hash);
