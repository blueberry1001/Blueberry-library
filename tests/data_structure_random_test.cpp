...
void test_sparse_table() {
  ...
  for (int n = 1; n <= 100; ++n) {
    ...
    for (int iteration = 0; iteration < 300; ++iteration) {
      ...
      ++right;
      assert(table.prod(left, right) == table.product(left, right));
      assert(table.prod(left, right) ==
             *std::min_element(values.begin() + left, values.begin() + right));
    }
  }
}

// Legacy value types require += and binary -, but need not provide -=.
struct AdditiveValue {
  long long value = 0;
  AdditiveValue& operator+=(const AdditiveValue& other) {
    value += other.value;
    return *this;
  }
  friend AdditiveValue operator-(AdditiveValue a, AdditiveValue b) {
    return {a.value - b.value};
  }
};

void test_fenwick_tree() {
  blueberry::FenwickTree<AdditiveValue> legacy(
      std::vector<AdditiveValue>{{2}, {-4}, {7}, {3}});
  assert(legacy.get(3).value == 3);
  assert(legacy.sum(1, 3).value == 3);
  legacy.add(3, {-5});
  assert(legacy.get(3).value == -2);
  for (int n = 0; n <= 100; ++n) {
...
void test_segment_tree() {
  ...
  for (int n = 0; n <= 40; ++n) {
    ...
    for (int iteration = 0; iteration < 300; ++iteration) {
      ...
      assert(tree.product(left, right) == tree.prod(left, right));
      if (n > 0 && rng() % 3 == 0) {
        const int index = static_cast<int>(rng() % n);
    ...
    assert(tree.all_prod() == tree.prod(0, n));
    assert(tree.all_product() == tree.all_prod());
  }
}
