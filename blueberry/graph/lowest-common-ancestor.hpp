...
class LowestCommonAncestor {
 public:
  ...
  // Short spelling; `kth_ancestor` remains the descriptive API.
...
/**
 * @brief Lowest Common Ancestor by Euler tour and a Sparse Table.
 *
 * Construction takes O(N log N) time and memory.  The Euler tour has 2N-1
 * entries, and `lca`/`distance` queries take O(1).  The input must be a
 * connected tree represented by an undirected adjacency list (or a
 * root-reachable parent-to-child list).
 */
class LowestCommonAncestorRMQ {
 private:
  using Item = std::pair<int, int>;  // (depth, vertex)
 ...
 public:
  explicit LowestCommonAncestorRMQ(const std::vector<std::vector<int>>& tree, int root = 0)
    ...
    std::vector<int> parent(n_, -1), next_edge(n_, 0), stack{root};
    parent[root] = root;
    depth_[root] = 0;
