...
以下の元監査はmain `4448ce5` 時点の履歴として保持します。PR #5〜#10を統合した
現在の状態・レビュー判定・検証結果は [INTEGRATION.md](INTEGRATION.md) を参照してください。

- 完了: 短縮APIと互換alias、SparseTableのflat配置、Fenwick get/lower_bound、rollback履歴の軽量化。
- 完了: O(1) RMQ LCA、順序付きHLD、jump、頂点・辺・部分木クエリ、公式verify追加。
- 完了: GCC/Clang × C++20/23のcompile/固有randomテストCI、benchmark runner。
- 現在: カタログ11ライブラリ、公開・互換入口13ヘッダ、公式verify20本。
- P0: カタログ外の旧6実装の安全な移行・隔離。旧Li Chao/2D Fenwick/rollbackの単体利用問題、
  ordered treapとimplicit treapの名称不一致は今回の統合で修復していません。
- P1: FPSの未検証操作、Dijkstraの境界・overflow前提、rollbackの境界条件を追加検証。
- P1: 計測済みflat SparseTableを基準に、線形空間RMQ、素数列挙、LCA、I/Oを同条件で比較。
  blocked診断版はqueryが遅いため、そのまま本体へ置換しません。
- P2: Wavelet Matrix、offline dynamic connectivity、文字列・幾何の不足分。
- P2: Fastest調査の旧CLIと新CLIは出力形式が異なるため当面併存。新規用途は
  `fetch_lc_submissions.py` に統一し、将来共通clientへ寄せます。
