...
初回実装の完了条件にFastestのソース調査・同等契約での比較測定・性能改善を含める。
ACだけで公開を決めず、構築/操作/I/O/メモリを分離して測り、明らかな不要処理や
未調査の大きな性能差を残さない。既知の改善候補は初回作業内で評価し、採否と
前後の測定ログを記録する。対応型・演算・問い合わせ順・計算量等の汎用性は維持する。
