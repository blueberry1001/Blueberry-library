...
class LinkCutTree {
  ...
  void pull(int v) {
    auto& a = nodes_[v];
    const int l = a.child[0], r = a.child[1];
    // Missing children contribute only the identity, so do not invoke op for them.
    if (l == -1) {
      if (r == -1) { a.forward = a.value; a.backward = a.value; }
      else {
        a.forward = op(a.value, nodes_[r].forward);
        a.backward = op(nodes_[r].backward, a.value);
      }
    } else if (r == -1) {
      a.forward = op(nodes_[l].forward, a.value);
      a.backward = op(a.value, nodes_[l].backward);
    } else {
      a.forward = op(op(nodes_[l].forward, a.value), nodes_[r].forward);
      a.backward = op(op(nodes_[r].backward, a.value), nodes_[l].backward);
    }
  }
  void reverse(int v) {
    if (v == -1) return;
  ...
  void rotate(int v) {
    ...
    // The promoted aggregate is rebuilt when demoted again or at the final root.
    pull(p);
  }
  void splay(int v) {
    if (auxiliary_root(v)) { push(v); return; }
    stack_.clear();
  ...
  void access(int v) {
    ...
    for (int x = v; x != -1; x = nodes_[x].parent) {
      splay(x);
      if (nodes_[x].child[1] != last) { nodes_[x].child[1] = last; pull(x); }
      last = x;
    }
    splay(v);
  }
